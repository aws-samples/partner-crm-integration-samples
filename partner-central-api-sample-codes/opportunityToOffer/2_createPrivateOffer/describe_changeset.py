...
def save_offer_info_to_shared_env(response):
    """
    Save OFFER_ID and OFFER_ARN to shared environment file if changeset succeeded
    
    Args:
        response: The response from describe_changeset
    """
    if response.get('Status') == 'SUCCEEDED':
        # Look for CreateOffer change to extract offer information
        change_set = response.get('ChangeSet', [])
        offer_id = None
        
        for change in change_set:
            if change.get('ChangeType') == 'CreateOffer':
                entity = change.get('Entity', {})
                entity_identifier = entity.get('Identifier', '')
                # Extract offer ID from entity identifier (format: offer-xxxxx@1)
                if '@' in entity_identifier:
                    offer_id = entity_identifier.split('@')[0]
                    break
        ...
        print(f"⚠ Changeset status is {response.get('Status')}, not saving offer information")
