...
import { decodeServerEntitiesInObject } from '../../../utils/commonUtils';
import {
  Container,
  Header,
  Form,
...
function SimulateReview() {
  ...
  const fetchOpportunity = async () => {
    try {
      ...
      // Decode server-encoded entities in all string fields
      const decodedPayload = decodeServerEntitiesInObject(payload);
      
      // Replace Id with Identifier
      decodedPayload.Identifier = decodedPayload.Id;
      delete decodedPayload.Id;
      
      // Remove fields that should not be included
      delete decodedPayload.CreatedDate;
      delete decodedPayload.OpportunityTeam;
      delete decodedPayload.RelatedEntityIdentifiers;
      delete decodedPayload.$metadata;
      
      setUpdatePayload(decodedPayload);
    } catch (error) {
