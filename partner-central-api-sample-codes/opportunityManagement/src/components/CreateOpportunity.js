...
import { saveOpportunityId } from '../utils/sessionStorage';
import {
  Container,
  Header,
  Form,
...
function CreateOpportunity() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        console.log('Submitting opportunity:', payload);
        ...
        if (response && response.Id) {
            const opportunityId = response.Id;
            saveOpportunityId(opportunityId);
            
            // Navigate to the AssociateOpportunity page with the new opportunity ID
            navigate(`/associate-opportunity/${opportunityId}`, { 
            state: { 
                opportunityId,
                catalog: formData.Catalog
            }
              ...
              <SpaceBetween size="l">
                ...
                <FormField label="Other Industry">
                  <Input
                    ...
                    onChange={({ detail }) => handleChange('OtherIndustry', detail.value)}
                  />
                </FormField>
          ...
          <SpaceBetween size="l">
            ...
            <Form>
              <SpaceBetween size="l">
                ...
                {/* Sales activities */}
                <FormField label="Sales activities" optional={true}>
                  <Select
                    selectedOption={
                      formData.SalesActivities && formData.SalesActivities.length > 0 
                        ? { value: formData.SalesActivities[0], label: formData.SalesActivities[0] }
                        : null
                    }
                    onChange={({ detail }) => 
                      handleChange('SalesActivities', detail.selectedOption ? [detail.selectedOption.value] : [])
                    }
                    options={salesActivitiesOptions}
                    placeholder="Choose sales activities"
                  />
                </FormField>
                ...
                <FormField 
                  ...
                  description="Business problem must have at least 20 characters up to a recommended maximum of 1,000 characters."
                >
                  <Textarea
                ...
                <FormField label="Estimated AWS monthly recurring revenue" errorText={formData.ExpectedAmount && !/^(?:(?:[1-9](\.\d+)?|\d{2,15}(\.\d+)?|1000000000000000(\.\d+)?)|0\.(?!0+$)\d+)$/.test(formData.ExpectedAmount) ? 'Invalid amount format' : ''}>
                  <Input
