...
function AssociateOpportunity() {
  ...
  const [engagementResponse, setEngagementResponse] = useState(null);
  const [associatePayload, setAssociatePayload] = useState(null);
  const [startEngagementPayload, setStartEngagementPayload] = useState(null);
  ...
  useEffect(() => {
    ...
    const generatePayloads = async () => {
        ...
        // Generate start engagement payload
        const clientToken = uuidv4();
        const newStartEngagementPayload = {
        Catalog: catalog,
        ...
        setStartEngagementPayload(newStartEngagementPayload);
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        // Step 2: Start engagement from opportunity task
        const clientToken = uuidv4();
        
        const startEngagementPayload = {
        Catalog: catalog,
        Identifier: opportunityId,
        ClientToken: clientToken,
        AwsSubmission: {
        ...
        setStartEngagementPayload(startEngagementPayload);
        
        console.log('Start engagement payload:', startEngagementPayload);
        
        // Create and send the command
        const startEngagementCommand = new StartEngagementFromOpportunityTaskCommand(startEngagementPayload);
        const startEngagementResponse = await client.send(startEngagementCommand);
        
        console.log('Start engagement response:', startEngagementResponse);
        
        // Display the response in a tabular format
        setEngagementResponse(startEngagementResponse);
  ...
  // Render the response in a tabular format
  const renderEngagementResponse = () => {
    if (!engagementResponse) return null;
    
    // Convert the response object to an array of key-value pairs
    const responseItems = Object.entries(engagementResponse).map(([key, value]) => ({
      key,
    ...
    return (
      <Container>
        <Header variant="h2">Engagement Response</Header>
        <Table
          columnDefinitions={[
            {
              id: "key",
  ...
  // Render the start engagement payload in a tabular format
  const renderStartEngagementPayload = () => {
    if (!startEngagementPayload) return null;
    ...
    return (
        ...
        <pre style={{ 
            ...
            {JSON.stringify(startEngagementPayload, null, 2)}
        </pre>
        </Container>
    );
  };
    ...
    return (
        ...
        <SpaceBetween size="l">
            ...
            <Box margin={{ top: 'l' }}>
              ...
              <Box variant="samp">{opportunityId}</Box>
            </Box>
            </Container>
            
            {renderAssociatePayload()}
            {renderStartEngagementPayload()}
            {renderEngagementResponse()}
            <Box padding={{ top: 'l' }}>
            <Button onClick={() => navigate('/opportunities')}>Back to Opportunities</Button>
            </Box>
        </SpaceBetween>
  ...
  return (
    <Container>
      <SpaceBetween size="l">
        <Header variant="h1">AssociateOpportunity and StartEngagementFromOpportunityTask</Header>
        ...
        <Container>
          ...
          <Form>
            <FormField label="Opportunity ID">
              <Input value={opportunityId} disabled />
            </FormField>
        ...
        <Container>
          <Header variant="h2">AssociateOpportunity with Solutions, AWS Products and AWS Marketplace Private Offer</Header>
          <Box margin={{ top: 'l' }}>
            <Form>
              <SpaceBetween size="l">
                <FormField>
        ...
        <SpaceBetween direction="horizontal" size="xs" alignItems="center">
          ...
          <Button variant="primary" onClick={handleSubmit} loading={loading}>{loading ? 'Processing...' : 'Associate and Submit Opportunity'}</Button>
        </SpaceBetween>
      </SpaceBetween>
    </Container>
  );
