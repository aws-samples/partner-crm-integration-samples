...
"""
Purpose:
API wrapper for creating AWS-originated opportunities
"""
...
def generate_random_title(base_title="New AWS Business Deal"):
    """
...
def create_opportunity_with_payload(payload_json, client_token=None):
    ...
    try:
        ...
        if client_token:
            ...
            create_opportunity_request["ClientToken"] = f"aws-web-ui-{uuid.uuid4()}"
        
        # Ensure Origin is set to AWS Referral
        create_opportunity_request["Origin"] = "AWS Referral"
        
        # Perform the API call
        logger.info("Making create_opportunity API call for AWS opportunity")
        response = partner_central_client.create_opportunity(**create_opportunity_request)
        logger.info(f"Successfully created AWS opportunity")
        ...
        logger.error(f"ClientError in create_aws_opportunity: {str(err)}")
        return err.response
    except Exception as e:
        # For other exceptions, maintain a simple format
        logger.error(f"Unexpected error in create_aws_opportunity: {str(e)}")
        return {"Error": str(e)}
...
def prepare_default_payload(json_payload):
    ...
    try:
        ...
        payload["ClientToken"] = f"aws-web-ui-{uuid.uuid4()}"
        
        # Ensure Origin is set to AWS Referral
        payload["Origin"] = "AWS Referral"
