...
def accept_engagement_invitation(partner_central_client, invitation_id):
    """Accept engagement invitation by ID"""
    ...
    try:
        # Perform an API call
        response = partner_central_client.accept_engagement_invitation(**accept_invitation_request)
        return response
...
def usage_demo():
    ...
    print("-" * 88)
    print("Accept Engagement Invitation.")
    print("-" * 88)
    print("Usage: python 0_4_accept_engagement_invitation.py [engagement_invitation_id]")
    print("  If engagement_invitation_id is not provided, it will be taken from shared_env.json or default")
    print("-" * 88)
    ...
    print(f"\nAccepting Engagement Invitation ID: {invitation_id}")
    print()
    ...
    response = accept_engagement_invitation(partner_central_client, invitation_id)
    if response:
        helper.pretty_print_datetime(response)
        print(f"\nSuccessfully accepted engagement invitation: {invitation_id}")
    else:
        print("Failed to accept engagement invitation")
