...
def get_parameters_from_args_and_env():
    ...
    if num_args >= 2:
        ...
        entity_identifier = sys.argv[1]
        opportunity_identifier = shared_env.get('OPPORTUNITY_ID')
        if not opportunity_identifier:
            print("ERROR: OPPORTUNITY_ID not found in shared_env.json")
            print("Please run step 5 first to create an opportunity, or provide both entity_identifier and opportunity_identifier as command line arguments")
            sys.exit(1)
        print(f"Using entity_identifier from command line: {entity_identifier}")
        print(f"Using opportunity_identifier from shared env: {opportunity_identifier}")
        
    else:
        # No parameters provided, use shared env
        entity_identifier = shared_env.get('SOLUTION_ID')
        opportunity_identifier = shared_env.get('OPPORTUNITY_ID')
        
        if not entity_identifier:
            print("ERROR: SOLUTION_ID not found in shared_env.json")
            print("Please run step 6 first to list solutions, or provide entity_identifier as command line argument")
            sys.exit(1)
            
        if not opportunity_identifier:
            print("ERROR: OPPORTUNITY_ID not found in shared_env.json")
            print("Please run step 5 first to create an opportunity, or provide opportunity_identifier as command line argument")
            sys.exit(1)
            
        print(f"Using parameters from shared env:")
        print(f"  entity_identifier: {entity_identifier}")
        print(f"  opportunity_identifier: {opportunity_identifier}")
