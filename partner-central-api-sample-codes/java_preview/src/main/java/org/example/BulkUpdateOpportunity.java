...
public class BulkUpdateOpportunity {
	...
	public static void main(String[] args) {
		...
		String jsonPayloadFilePath = "bulkUpdateJson";
		
		List<String> allFiles = getAllFilenamesFromDir(jsonPayloadFilePath);
		
		for (String inputFile : allFiles) {
			try {
				UpdateOpportunityResponse response = updateOpportunity(jsonPayloadFilePath + "/" + inputFile);
	...
	public static List<String> getAllFilenamesFromDir (String dirPath) {
		List<String> allFilenames = new ArrayList<String>();
		
		try {
			Path resourceDirectory = Paths
					.get(BulkUpdateOpportunity.class.getClassLoader().getResource(dirPath).toURI());
			...
			try (Stream<Path> paths = Files.walk(resourceDirectory)) {
				paths.filter(Files::isRegularFile).filter(path -> path.toString().endsWith(".json"))
						.forEach(path -> allFilenames.add(path.getFileName().toString()));
			} catch (IOException e) {
				e.printStackTrace();
			}
...
private static GetOpportunityResponse getResponse(String opportunityId) {
	...
	public static UpdateOpportunityResponse updateOpportunity(String inputFile) {
		...
		if (response != null 
			...
			System.out.println("Not a " + OPPORTUNITY_ORIGIN + " opportunity");
			return null;
		}
    }
